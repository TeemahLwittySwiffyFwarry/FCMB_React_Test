...
export default function RegistrationPage() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await registerUser(sanitizedName, sanitizedJob);
    } catch (err) {
      ...
      <div className="w-full md:w-1/2 flex items-center justify-center px-6 lg:px-28 py-6">
        <div className="w-full max-w-md rounded-2xl p-8 space-y-8 relative">
          <div className="flex justify-center mb-20">
            <img src="/fcmb_logo.png" alt="FCMB Logo" className="w-24 h-24 object-contain" />
          </div>
          ...
          <div className="flex justify-center mt-2">
            <div className="flex justify-between items-center bg-gray-500 bg-opacity-60 rounded-[20px] w-[290px] h-[45px] p-1 shadow-inner">
              <button
                type="button"
                className="flex-1 py-2 px-3 rounded-full bg-white text-purple-800 font-semibold text-sm shadow-md transition-all duration-300"
              >
                Personal
              </button>
              <button
                type="button"
                className="flex-1 mx-1 py-1 rounded-full text-white/80 hover:text-white font-semibold text-sm transition-all duration-300"
              >
                Business
              </button>
            </div>
